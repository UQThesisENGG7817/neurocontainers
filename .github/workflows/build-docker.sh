...
docker build . --file ${IMAGENAME}.Dockerfile --tag $IMAGEID:$SHORT_SHA --cache-from $IMAGEID --label "GITHUB_REPOSITORY=$GITHUB_REPOSITORY" --label "GITHUB_SHA=$GITHUB_SHA" --network=host
...
then
    ...
    sudo apt-get update
    sudo apt-get install -y apptainer rclone
fi
...
echo "[DEBUG] Attempting upload to Nectar Object Storage ... SKIP Temporarily"
# rclone copy --progress $IMAGE_HOME/${IMAGENAME}_${BUILDDATE}.simg nectar:/neurodesk/temporary-builds-new

echo "[DEBUG] Attempting upload to AWS Object Storage ... SKIP Temporarily"
...
sudo ln -s -f /run/docker/docker.sock /var/run/docker.sock
